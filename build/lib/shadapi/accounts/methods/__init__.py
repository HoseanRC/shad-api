...
class Maker:
    ...
    async def request(self, method, data):
        ...
        status = loads(self.crypto.decrypt(response.get('data_enc'))).get('status')
        
        if status == 'OK':
            return loads(self.crypto.decrypt(response.get('data_enc')))

        elif status == 'ERROR_GENERIC' or status == 'ERROR_ACTION':
            status_det = loads(self.crypto.decrypt(response.get('data_enc'))).get('status_det')
...
class Methods:
    ...
    async def getMessages(self, object_guid, sort, id):
        data = {"object_guid": object_guid, "sort": sort}
        if sort == "FromMax":
            data["max_id"] = int(id)
        elif sort == "FromMin":
            data["min_id"] = int(id)
        else:
            raise InvalidInput('sort option should be one of \"FromMax\" or \"FromMin\"')
        return (await self.make.request(
            method='getMessages',
            data=data,
        )).get("data").get("messages")

    async def getMessagesFromMax(self, object_guid, id):
        return await self.getMessages(object_guid, "FromMax", id)

    async def getMessagesFromMin(self, object_guid, id):
        return await self.getMessages(object_guid, "FromMin", id)
    ...
    async def getChats(self, start_id=None):
        ...
        return response.get('data').get('chats')
    ...
    async def _baseSendFiles(self, file_type, file_bytes, **kwargs):
        ...
        access_hash_rec = await self.make.uploadFile(
            upload_url = rsf.get('upload_url'),
            access_hash_send = rsf.get("data").get('access_hash_send'),
            file_id = rsf.get('id'),
    ...
    async def download(self, object_guid, message_id, save=False):
        ...
        if 'file_inline' in list(message_info.keys()):
            ...
            makeURL = f'https://shadmessenger{dc_id}.iranlms.ir/GetFile.ashx'
            data = b''
